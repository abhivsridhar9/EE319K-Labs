;****************** main.s ***************
; Program Written By: Ryan Root and Abhi Sridhar
...
; PA4, PE3-PE0 out logic analyzer to TExasDisplay
 ...
 ;place your initializations here
	  ...
	  BL   Heartbeat_Init ; initialize onboard LED
;**********************
...
no    MOV  R5,R4   ; setup for next time
      BL   Stepper_Step               
      LDR  R0,=1600000
      BL   Wait  ; time delay fixed but not accurate .04  
      B    loop
...
;Check to see if buffers are full
	CMP R0, R1
	BEQ Done
	CMP R2, R3
	BEQ Done
...
Done
		POP  {R0-R6,PC}
;43 Instructions (1075 ns) -- 47 w/LED Toggle (1175 ns)
;Time Between Calls (~.08 s)
;Estimated Intrusiveness (0.00134372%) -- w/LED Toggle (0.00146872%)

;Initializes the Heartbeat/LED Indicator
Heartbeat_Init
		PUSH {R0-R2, LR}
